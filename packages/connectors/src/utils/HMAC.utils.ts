...
export function HMACSHA256Base64(key: string, value: string): string {
  const encoder = new TextEncoder()
  const hmacValue = hmac(encoder.encode(key), encoder.encode(value))
  const base64 = base64ArrayBuffer(hmacValue)
  return base64
}
